...
class Image extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('hotel_id, room_id', 'required'),
			array('hotel_id, room_id', 'numerical', 'integerOnly'=>true),
//			array('link', 'length', 'max'=>500),
                      array('avatarUpload', 'file', 'types' => 'jpg, gif, png', 'allowEmpty' => TRUE ),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, link, hotel_id, room_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'room' => array(self::BELONGS_TO, 'Rooms2', 'room_id'),
		);
	}
	 ...
	 */
        ...
        {
            $this->avatarUpload = CUploadedFile::getInstance($this, 'link');
            if (isset($this->avatarUpload)) {
                ...
                $this->link = '/Upload/' . date('Y-m-d') . '/' . $fileName; //Lưu path vào csdl
            }
